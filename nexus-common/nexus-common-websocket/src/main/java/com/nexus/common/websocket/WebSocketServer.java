package com.nexus.common.websocket;
...
import com.nexus.common.websocket.domain.Message;
import com.nexus.common.websocket.enums.MessageTypeEnum;
...
public class WebSocketServer {
    ...
    public void error(Throwable throwable) {
        ...
        log.error("====> WebSocket 发生错误，错误信息", throwable);
    }
    ...
    public void close(Session session, @PathParam("token") String token) {
        try {
            ...
            log.error("====> WebSocket 连接关闭失败，失败原因", e);
    ...
    private void closeSession(Session session, CloseReason.CloseCode closeCode, String reason) {
        try {
            ...
            log.debug("====> WebSocket 连接关闭失败，失败原因", e);
        }
    }
    ...
    public void sendOnlineNotice(Long onlineUserId) {
        ...
        sendPrivateMessage(onlineUserId, JSON.toJSONString(message));
    }
    ...
    public static void sendPrivateMessage(Long getterUserId, Message message) {
        try {
            ...
            socketMap.get(getterUserId).session.getBasicRemote().sendText(JSON.toJSONString(message));
        } catch (IOException e) {
            log.error("====> 私息发送异常", e);
        }
    }
    ...
    public static void sendPrivateMessage(Session getterSession, Message message) {
        try {
            ...
            getterSession.getBasicRemote().sendText(JSON.toJSONString(message));
        } catch (IOException e) {
            log.error("====> 私息发送异常", e);
        }
    }
